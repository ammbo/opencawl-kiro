 ...
 * Approves a waitlist entry by updating its status to 'approved'.
 */
export async function onRequestPost(context) {
  ...
  try {
    ...
    await db
      .prepare("UPDATE waitlist SET status = 'approved' WHERE id = ?")
      .bind(waitlist_id)
      .run();
    ...
    return json(
      { error: { code: 'INTERNAL_ERROR', message: 'Failed to approve waitlist entry' } },
      500,
    );
  }
}
